...
export function reanchorPositionTracks(clip: THREE.AnimationClip, vrm: VRM) {
  ...
  // Use the current local hips position as the anchor; using world coordinates would introduce unwanted drift
  // when the model root is moved (pan/center) or refit.
  const currentHipLocal = hipsNode.position.clone();
  ...
  const delta = first.sub(currentHipLocal);
