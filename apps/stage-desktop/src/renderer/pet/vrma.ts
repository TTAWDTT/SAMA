 ...
 */
export function reanchorPositionTracks(clip: THREE.AnimationClip, vrm: VRM, anchorHipsLocal?: THREE.Vector3) {
  const hipsNode = (vrm as any).humanoid?.getNormalizedBoneNode?.("hips");
  if (!hipsNode) return;

  // Tracks generated by `createVRMAnimationClip` target `${nodeName}.position` which is LOCAL position.
  // Anchor against a stable local reference (preferably the model's rest hips local position).
  const baseHipLocal = (anchorHipsLocal ? anchorHipsLocal.clone() : hipsNode.position.clone()) as THREE.Vector3;
  ...
  const delta = first.sub(baseHipLocal);
